...
public partial class _viewPatients : System.Web.UI.Page
{
    ...
    public static string newDoctorShift(shifts shift)
    {
        ...
        using (SqlCommand cmd = new SqlCommand("insert into Endoresment_Doctors_Shifts  (Doctor_id,Doctor_name,Receive_from,Receive_name, Shift, Shift_date, Confirm, Entry_user) values(@Doctor_id, @Doctor_name,@Receive_from,@Receive_name,@Shift,@Shift_date,@Confirm, @Entry_user) SELECT CAST(scope_identity() AS int)", con))
        {
            cmd.Parameters.Add("@Doctor_id", SqlDbType.Int).Value = shift.Doctor_id;
            cmd.Parameters.Add("@Doctor_name", SqlDbType.VarChar).Value = shift.Doctor_name;
    ...
    public static string completeDoctorShift(shifts shift)
    {
        ...
        using (SqlCommand cmd = new SqlCommand("update Endoresment_Doctors_Shifts set Completed = 1 where id = @id", con))
        {
    ...
    public static string confirmShift(shifts shift)
    {
        ...
        using (SqlCommand cmd = new SqlCommand("update Endoresment_Doctors_Shifts set Confirm = 1 where id = @id", con))
        {
            cmd.Parameters.Add("@id", SqlDbType.Int).Value = shift.id;
            SqlDataReader idr = cmd.ExecuteReader();
        }
    ...
    public static string getShiftsData(DoctorData data)
    {
        ...
        using (SqlCommand cmd = new SqlCommand("select top(1) * from Endoresment_Doctors_Shifts where Doctor_id = @Doctor_id and (Shift_date >= DATEADD(day,-2, GETDATE())) order by id ASC", con))
        {
            cmd.Parameters.Add("@Doctor_id", SqlDbType.Int).Value = data.Dr_Code;
            SqlDataReader idr = cmd.ExecuteReader();
    ...
    public static string getDoctorData(DoctorData data)
    {
        ...
        using (SqlCommand cmd = new SqlCommand("select * from Doctors_Data where Dr_Code = @Dr_Code", con))
        {
    ...
    public static string getDoctorsData(DoctorData data)
    {
        ...
        using (SqlCommand cmd = new SqlCommand("select * from Doctors_Data where Spcy_Description = @Spcy_Description and Type = 'Resident'", con))
        {
            cmd.Parameters.Add("@Spcy_Description", SqlDbType.VarChar).Value = data.Spcy_Description;
            SqlDataReader idr = cmd.ExecuteReader();
    ...
    populateDoctorDataLisst(SqlDataReader idr, SqlConnection con)
    {
        ...
        while (idr.Read())
        {
            DoctorDataI
                .Add(new DoctorData {
					Spcy_Description = idr["Spcy_Description"] != DBNull.Value ? Convert.ToString(idr["Spcy_Description"]) : String.Empty,
					Type = idr["Type"] != DBNull.Value ? Convert.ToString(idr["Type"]) : String.Empty,
					Dr_Code = idr["Dr_Code"] != DBNull.Value ? Convert.ToInt32(idr["Dr_Code"]) : 0,
					DR_Name = idr["DR_Name"] != DBNull.Value ? Convert.ToString(idr["DR_Name"]) : String.Empty,
                });
        }
    ...
    public static string getPatientsData(DoctorData data)
    {
        ...
        using (SqlCommand cmd = new SqlCommand("select * from Endorsement_PatientData where Consultant_id = @Consultant_id", con))
        {
            cmd.Parameters.Add("@Consultant_id", SqlDbType.Int).Value = data.Dr_Code;
            SqlDataReader idr = cmd.ExecuteReader();
    ...
    public static string getPatientsData2(DoctorData data)
    {
        ...
        using (SqlCommand cmd = new SqlCommand("select * from Endorsement_PatientData where Specialty = @Spcy_Description", con))
        {
            cmd.Parameters.Add("@Spcy_Description", SqlDbType.VarChar).Value = data.Spcy_Description;
            SqlDataReader idr = cmd.ExecuteReader();
    ...
    public static string getConsultationsData(DoctorData data)
    {
        ...
        using (SqlCommand cmd = new SqlCommand("select * from Endorsement_PatientData where id in (select top(1) Patient_id from Endorsement_PatientFollow where Consultaion = @Consultant_id order by id ASC)", con))
        {
            cmd.Parameters.Add("@Consultant_id", SqlDbType.Int).Value = data.Dr_Code;
            SqlDataReader idr = cmd.ExecuteReader();
        ...
        while (idr.Read())
        {
            Endorsement_PatientDataI
                .Add(new Endorsement_PatientData {
					...
					Consultant_id =  idr["Consultant_id"] != DBNull.Value ? Convert.ToInt32(idr["Consultant_id"]): 0,
