...
public partial class _Handover : System.Web.UI.Page
{
    ...
    public static string getUnitDashData(user chargeNurse)
    {
        ...
        using (
            SqlCommand cmd =
                new SqlCommand("SELECT * from Endorsement_UnitsDashboard where Shift_date >= DATEADD(day,-1, GETDATE()) and Unit_id in (select TOP(20) Unit_id from Endorsement_UnitsDashboard where Receive_ChargeNurse_id = @Receive_ChargeNurse_id and Shift_date >=   DATEADD(day,-1, GETDATE()) order by id desc) order by id desc",
                    con)
        )
        {
            cmd.Parameters.Add("@Receive_ChargeNurse_id", SqlDbType.Int).Value =
                chargeNurse.Emp_id;
    ...
    public static string getPatientsData(user chargeNurse)
    {
        ...
        using (
            SqlCommand cmd =
                new SqlCommand("select * from Endorsement_PatientData where Patient_Status = 1 and  Unit in (select TOP(20) Unit_name from Endorsement_UnitsDashboard where Receive_ChargeNurse_id = @Receive_ChargeNurse_id order by id desc)",
                    con)
        )
        {
            cmd.Parameters.Add("@Receive_ChargeNurse_id", SqlDbType.Int).Value =
    ...
    public static string getShiftData(Endorsement_shiftData data)
    {
        ...
        using (
            SqlCommand cmd =
                new SqlCommand("select * from Endorsement_PatientFollow where Patient_id = @Patient_id order by id",
                    con)
        )
        {
