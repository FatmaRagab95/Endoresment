<template>
    <div class="viewPatients" v-if='DoctorData'>
        ...
        <div class='endoresment-options pt-3 text-center border rounded' v-if='DoctorData.Type.trim() =="Resident"'>
            <div class="row align-items-center border-bottom border-white pt-3 pb-3">
                ...
                <div class="col-md-6">
                    <div class="receive-shift" v-if='Shifts.length == 0'>
                        ...
                        <button v-if='unConfirmedShifts.length == 0'
        ...
        <div class="container">
            ...
            <h1 class='pt-5 pb-3  border-bottom mainColor'>
                ...
                {{DoctorData.Spcy_Description}} patients list</h1>
                ...
                <div class="col-md-4" v-for='(patient) in patients' :key='patient.id'>
                    <div class='details'>
                        <div class="inner shadow">
                            ...
                            <span class='speciality badge font-weight-normal' v-if='DoctorData.Type =="Consultant"'>
                                {{patient.Consultant_id ==  user.Emp_id ? 'Primary' : 'Consultation'}}
                            </span>
                            <span class='speciality badge font-weight-normal' v-else>
                                {{patient.Specialty.trim() ==  DoctorData.Spcy_Description.trim() ? 'Primary' : 'Consultation'}}
                            </span>
            ...
            <div class="card text-center" v-else>
                <p class='mt-3 alert'><i class='fa fa-warning text-warning'></i> 
                You have no patients yet!</p>
            </div>
        </div>
                                ...
                                <div class="cu-field">
                                    ...
                                    <div class="f-select">
                                        <select class="form-control form-control-sm" required v-model='newShift.Receive_from'>
                                            ...
                                            :key='doctor.Dr_Code'>{{doctor.DR_Name}}</option>
                                        </select>
                                    </div>
                                </div>
                        ...
                        <form>
                            <div class="custom-form">
                                <div class="cu-field">
                                    ...
                                    <div class="f-select">
                                        <select class="form-control form-control-sm" required v-model='newShift.Doctor_id'>
                                            <option value=''>...</option>
                                            <option v-for='doctor in Doctors' 
                                            :value='doctor.Dr_Code' 
                                            :key='doctor.Dr_Code'>{{doctor.DR_Name}}</option>
        ...
        insertShift() {
            if ($(".modal.show form").is(":valid")) {
                ...
                if (!that.newShift.Doctor_id) {
                    ...
                    that.newShift.Receive_name = that.Doctors.filter(x => x.Dr_Code == that.newShift.Receive_from)[0].DR_Name;
                ...
                // if it's handover
                    ...
                    that.newShift.Doctor_name = that.Doctors.filter(x => x.Dr_Code == that.newShift.Doctor_id)[0].DR_Name;
